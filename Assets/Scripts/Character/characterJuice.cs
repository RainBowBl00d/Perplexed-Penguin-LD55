...
public class characterJuice : MonoBehaviour
{
    ...
    {
        if (!PlayerStats.instance.playerGrounded && PlayerStats.instance.onGround)
        {
            ...
            if (!PlayerStats.instance.landSqueezing && landSqueezeMultiplier > 1)
            {
        ...
        else if (PlayerStats.instance.playerGrounded && !PlayerStats.instance.onGround)

        {
            // Player has left the ground, so stop playing the running particles
            PlayerStats.instance.playerGrounded = false;
            moveParticles.Stop();
        }
    }
    ...
    public void jumpEffects()
    {
        ...
        if (!PlayerStats.instance.jumpSqueezing && jumpSqueezeMultiplier > 1)
        {
    ...
    IEnumerator JumpSqueeze(float xSqueeze, float ySqueeze, float seconds, float dropAmount, bool jumpSqueeze)
    {
        ...
        if (jumpSqueeze) { PlayerStats.instance.jumpSqueezing = true; }
        else { PlayerStats.instance.landSqueezing = true; }
        PlayerStats.instance.squeezing = true;
        ...
        if (jumpSqueeze) { PlayerStats.instance.jumpSqueezing = false; }
        else { PlayerStats.instance.landSqueezing = false; }
    }
